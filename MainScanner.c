...
thrn_int errorNumber;				/* Run-time error number = 0 by default (ANSI) */

/* External objects */
extern thrn_int line; /* Source code line numbers - defined in scanner.c */
extern Token tokenizer(thrn_nul);
 ...
 * -------------------------------------------------------------
 */
thrn_nul printScannerError(thrn_chr* fmt, ...);
thrn_nul displayScanner(Buffer* ptrBuffer);
thrn_lng getScannerFilesize(thrn_chr* fname);
